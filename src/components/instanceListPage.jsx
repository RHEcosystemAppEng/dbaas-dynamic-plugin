...
const InstanceListPage = () => {
  ...
  const dbProviderTitle = (<div>Connect {dbProviderName} <Label className="ocs-preview-badge extra-left-margin">{t('Alpha')}</Label></div>);
  ...
  const parsePayload = (responseJson) => {
    ...
    if (responseJson.items?.length > 0) {
      ...
      filteredInventories.forEach((inventory, index) => {
        ...
        if (inventory.status?.conditions[0]?.status !== "False" && inventory.status?.conditions[0]?.type === "SpecSynced") {
          ...
          <React.Fragment>
            {(fetchInstancesFailed || noInstances) ?
              <EmptyState>
                ...
                <EmptyStateBody>
                  Database instances could not be fetched. Please try again.
                </EmptyStateBody>
                <Button variant="primary" onClick={handleTryAgain}>Try Again</Button>
                <EmptyStateSecondaryActions>
                  <Button variant="link" onClick={handleCancel}>Close</Button>
                </EmptyStateSecondaryActions>
              </EmptyState>
              :
              <React.Fragment>
                <FormGroup label="Provider Account" fieldId="provider-account" className="provider-account-selection">
                  <FormSelect value={selectedInventory.name} onChange={handleInventorySelection} aria-label="Provider Account">
                ...
                <FormSection fullWidth flexLayout className="no-top-margin">
                  <InstanceTable isLoading={!showResults} data={selectedInventory} isSelectable={selectedInventory?.instances?.length > 0 && filteredInstances.length > 0} filteredInstances={filteredInstances} />
                </FormSection>
              </React.Fragment>
            }
          </React.Fragment>
